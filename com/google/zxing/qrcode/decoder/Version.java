...
public final class Version
{
  ...
  private Version(int paramInt, int[] paramArrayOfInt, ECBlocks[] paramArrayOfECBlocks)
  {
    ...
    ECB[] arrayOfECB = paramArrayOfECBlocks[0].getECBlocks();
    int k = arrayOfECB.length;
    int m = 0;
    while (i < k)
    {
      ECB localECB = arrayOfECB[i];
      m += localECB.getCount() * (j + localECB.getDataCodewords());
      i++;
    }
  ...
  public static Version getProvisionalVersionForDimension(int paramInt)
  {
    ...
    //   3: iconst_1
    //   4: if_icmpeq +7 -> 11
    //   7: invokestatic 116	com/google/zxing/FormatException:getFormatInstance	()Lcom/google/zxing/FormatException;
    //   10: athrow
    //   11: iload_0
    //   12: bipush 17
    //   14: isub
    ...
    //   17: istore_1
    //   18: iload_1
    //   19: invokestatic 101	com/google/zxing/qrcode/decoder/Version:getVersionForNumber	(I)Lcom/google/zxing/qrcode/decoder/Version;
    //   22: astore_2
    //   23: aload_2
    //   24: areturn
    //   25: invokestatic 116	com/google/zxing/FormatException:getFormatInstance	()Lcom/google/zxing/FormatException;
    //   28: athrow
    //
    // Exception table:
    //   from	to	target	type
    //   18	23	25	java/lang/IllegalArgumentException
  }
  ...
  BitMatrix buildFunctionPattern()
  {
    ...
    int j = i - 8;
    localBitMatrix.setRegion(j, 0, 8, 9);
    localBitMatrix.setRegion(0, j, 9, 8);
    int k = this.alignmentPatternCenters.length;
    for (int m = 0; m < k; m++)
    {
      int i2 = -2 + this.alignmentPatternCenters[m];
      for (int i3 = 0; i3 < k; i3++)
        if (((m != 0) || ((i3 != 0) && (i3 != k - 1))) && ((m != k - 1) || (i3 != 0)))
          localBitMatrix.setRegion(-2 + this.alignmentPatternCenters[i3], i2, 5, 5);
    }
    int n = i - 17;
    localBitMatrix.setRegion(6, 9, 1, n);
    localBitMatrix.setRegion(9, 6, n, 1);
    if (this.versionNumber > 6)
    {
      int i1 = i - 11;
      localBitMatrix.setRegion(i1, 0, 3, 6);
      localBitMatrix.setRegion(0, i1, 6, 3);
    }
